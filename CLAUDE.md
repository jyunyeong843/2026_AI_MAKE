...
- **실시간 내비게이션**: GPS를 계속 추적해 현재 위치 점을 따라 이동시키고, 남은 거리·시간을
  갱신하며, 경로 이탈/도착을 감지해 안내한다(아래 **실시간 내비게이션 동작** 참고).

## 2. 지도 룰 (경로 결정 규칙)

1. 사용자의 GPS 위치를 기준으로 반경 1km 이내의 공공기관 목록을 출력한다.
   - 대상 시설: 주민센터, 동사무소, 구청, 복지관, 문화센터 등 어르신이 방문할 가능성이 높은 공공·복지 시설로 한정한다.
2. 사용자가 목적지를 선택하면 **osmnx 그래프**에서 기본 최단 경로를 탐색한다. (네이버 지도 API는 사용하지 않음 — 아래 **기술 스택** 참고)
...
- **남은 거리/시간**: 추천 경로 폴리라인에서 현재 위치와 가장 가까운 구간을 찾아 그 이후 거리를 합산한다.
  예상 시간은 **남은 거리 ÷ 1.0 m/s**(핵심 상수의 보행 속도)로 계산해 표시한다.
...
- ⚠️ 위 거리/이탈 계산은 **표시·안내용 단순 산수**다. 경사/강수/비용 등 경로 **판단**은
  하지 않으며(그건 채점 엔진의 몫), 정확도는 백엔드가 주는 좌표 품질에 종속된다.
...
> TTS 추가 안내: 경로 이탈 시 "경로를 벗어났습니다. 원래 길로 돌아가세요", 도착 시 "목적지에 도착했습니다" 도 출력한다.
> 구현은 브라우저 Web Speech API(`speechSynthesis`, `ko-KR`), 코드 `frontend/src/lib/tts.js`.
...
backend/                      # Express(:5000). 현재는 데모용 mock 엔드포인트
```

- 프론트는 API를 **`/api`** 로만 호출한다. `frontend/vite.config.js` 에서 `/api → http://localhost:5000` 프록시.
- **백엔드 연동 계약**: 프론트는 `POST /api/route/score` 에 **7장 입력 스키마**를 보내고 **출력 스키마**를 받기를 기대한다.
  아직 해당 엔드포인트가 없으면 `frontend/src/lib/api.js` 의 로컬 mock 채점기(6장 비용 공식 그대로 구현)로 폴백한다.
  → 백엔드가 이 계약대로 응답하면 **프론트 수정 없이** 실제 경로가 그려진다.
- 현재 `backend/server.js` 의 `/api/navigation/route` 는 스키마가 달라 아직 연결돼 있지 않다.
